...
public interface DailySalesRepository extends JpaRepository<DailySales, Long> {
    ...
    @Query("""
            SELECT DISTINCT d FROM DailySales d
            JOIN FETCH d.product p
            JOIN FETCH d.company c
            WHERE d.company.id = :companyId
              AND d.salesDate = :salesDate
            ORDER BY p.category ASC, p.productCode ASC
            """)
    ...
    @Query("""
            ...
            JOIN FETCH d.company c
            WHERE d.company.id = :companyId
              AND YEAR(d.salesDate) = :year
              AND MONTH(d.salesDate) = :month
            ORDER BY p.category ASC, p.productCode ASC, d.salesDate ASC
            """)
    ...
    @Query("""
            ...
            ORDER BY c.name ASC, p.category ASC, p.productCode ASC
            """)
    ...
    @Query("""
            ...
            ORDER BY d.salesDate ASC, c.name ASC
            """)
    ...
    @Query("""
            ...
            ORDER BY d.salesDate ASC, p.category ASC, p.productCode ASC
            """)
    ...
    @Query("""
            ...
            WHERE d.company.id = :companyId
              AND d.product.id = :productId
              AND d.salesDate >= :startDate
              AND d.salesDate <= :endDate
            ORDER BY d.salesDate ASC
            """)
